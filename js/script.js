...
(function () {
  ...
  function applyFieldTheme() {
    ...
    stars.forEach((s, i) => {
      s.style.setProperty(
        "--c",
        sp[(Math.random() * sp.length) | 0] + (light ? 0.35 : 0.35 + Math.random() * 0.6).toFixed(2) + ")",
      );
    });
...
(function () {
  ...
  function build() {
    const reduced = window.matchMedia("(prefers-reduced-motion: reduce)").matches;
    const palette =
      reduced || document.documentElement.getAttribute("data-theme") === "light"
...
(function () {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const left =
      Math.random() < 0.7 ? (Math.random() < 0.5 ? Math.random() * 14 : 86 + Math.random() * 10) : Math.random() * 100;
